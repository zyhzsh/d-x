...
const processChange = debounce((factorId,value) => saveInput(factorId,value));
    ...
    <Grid key={factor.id} item container sx={{ mt:1, height: '40px'}}>
      <Grid item xs={7} md={8}  sx={{ p:1,  height: '40px' ,display: 'flex', justifyContent: 'center',alignItems:'center'}}>
        <TextField 
          ...
          />
          {onUpdateOptionId===factor.id&&<IconButton onClick={DeleteFactorHandler}><DeleteOutlineIcon color='primary'/></IconButton>}
      </Grid>
  ...
  return (
      ...
      <Grid item xs={5} md={4}  sx={{ height: '40px' ,display: 'flex',pl:'4px',justifyContent: 'center',alignItems:'center'}}><Button onClick={AddNewFactorHandler} fullWidth variant='outlined'><AddCircleOutlineIcon /></Button></Grid>      
      </Grid>
